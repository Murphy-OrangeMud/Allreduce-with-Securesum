...
void allreduce(const detail::AllreduceOptionsImpl& opts) {
  ...
  switch (opts.algorithm) {
    ...
    case detail::AllreduceOptionsImpl::RING:
      ring<T>(opts, reduceInputs, broadcastOutputs);
      break;
...
template<typename T>
void generateRandom(T *ptr, size_t nelem) {
...
template<>
void generateRandom(float *ptr, size_t nelem) {
  static unsigned counter = 0;
  static std::mt19937 engine;
  if (!counter) engine.seed(time(0));
  for (int i = 0; i < nelem; i++) {
    ptr[i] = engine() / 998244353.0;
  }
  counter++;
}

template<>
void generateRandom(double *ptr, size_t nelem) {
  static unsigned counter = 0;
...
template<>
void generateRandom(int8_t *ptr, size_t nelem) {
  static int counter = 0;
  static std::mt19937_64 engine;
  if (!counter) engine.seed(time(0));
  for (int i = 0; i < nelem; i++) {
    ptr[i] = (int8_t)(engine() % 256);
  }
  counter++;
}

template<>
void generateRandom(uint8_t *ptr, size_t nelem) {
  static int counter = 0;
  static std::mt19937_64 engine;
  if (!counter) engine.seed(time(0));
  for (int i = 0; i < nelem; i++) {
...
template<>
void generateRandom(int32_t *ptr, size_t nelem) {
  static int counter = 0;
  static std::mt19937_64 engine;
  if (!counter) engine.seed(time(0));
  for (int i = 0; i < nelem; i++) {
    ptr[i] = engine() % 998244353;
  }
  counter++;
}

template<>
void generateRandom(int64_t *ptr, size_t nelem) {
  static int counter = 0;
  static std::mt19937_64 engine;
  if (!counter) engine.seed(time(0));
  for (int i = 0; i < nelem; i++) {
...
void ring(
  ...
  // add noise
  auto startOffset = computeReduceScatterOffsets(0);

  ssize_t nelem = std::min(
		  	    (ssize_t)numSegmentsPerRank * (ssize_t)segmentBytes,
		       	(ssize_t)totalBytes - (ssize_t)startOffset.sendOffset) / (ssize_t)opts.elementSize;
  
  T *randomBuffer = nullptr;

  for (int i = 0; i < numSegmentsPerRank; i++) {
  ...
  if (nelem > 0) {
    randomBuffer = static_cast<T*>(malloc(nelem * opts.elementSize));
    generateRandom(randomBuffer, nelem);
    auto tmp_ptr = static_cast<T*>(out[0]->ptr);
    for (int i = 0; i < nelem; i++) {
      tmp_ptr[startOffset.sendOffset / opts.elementSize + i] += randomBuffer[i];
    }
  }
  ...
  // Push to the next(original) GPU to clear noise
  for (auto i = 0; i < numSegmentsPerRank + 2; i++) {
    if (i >= 2) {
      auto prev = computeReduceScatterOffsets(numSegments - numSegmentsPerRank + i - 2);
      if (prev.recvLength > 0) {
    ...
    if (i < numSegmentsPerRank) {
      auto cur = computeReduceScatterOffsets(numSegments - numSegmentsPerRank + i);
      if (cur.recvLength > 0) {
